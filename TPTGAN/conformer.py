...
class TSCB(nn.Module):
    ...
    def forward(self, x_in):
        b, c, t, f = x_in.size()
        x_in = x_in.permute(0, 3, 2, 1).contiguous().view(b*f, t, c)
        x_in = self.time_conformer(x_in) + x_in
        x_in = x_in.view(b, f, t, c).permute(0, 2, 1, 3).contiguous().view(b*t, f, c)
        x_in = self.freq_conformer(x_in) + x_in
        x_in = x_in.view(b, t, f, c).permute(0, 3, 1, 2)
